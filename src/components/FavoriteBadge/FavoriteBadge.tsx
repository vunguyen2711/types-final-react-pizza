...
import { Avatar, Badge, Popover, Space, Divider, Button, List } from "antd";
import { HeartOutlined } from "@ant-design/icons";
import { useAppDispatch, useAppSelector } from "../../redux/hook";
import * as S from "./style";
import { getFavoriteState,toggleFavoriteId } from "../../redux/features/FavoriteProDucts/FavoriteProductsSlice";
  ...
  const popoverContent = !isLogin ? (
    ...
    <Space direction="vertical">
        {favoriteProducts?.map((item) => (
          <List className="favorite__items" key={item.id} style={{ width: "100%" }}>
            <List.Item>
          ...
          to={`${RoutesPath.FOODDETAIL}/${item.id}`}
            ...
            <Button onClick={()=>dispatch(toggleFavoriteId(item.id))}>Delete</Button>
            </List.Item>
